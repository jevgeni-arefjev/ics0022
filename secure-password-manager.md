...
This app is:
- Desktop native
- Single-user (e.g. no teams or secret sharing)
...
The app consists of 5 modules with responsibilities split between them. The goal of this app is to securely store passwords and protect against memory leaks, injection attacks and other cryptographic attacks.

| Module | Responsibility |
| --- | --- |
| CLI | Command parsing, terminal output, one-field reveal, clipboard |
...
| Storage | Read/write vault, atomic replace, version checks |
| Backup | Ciphertext-only export, verify, retention |

```mermaid
flowchart LR
  CLI --> UM[User management] -->|master password| ENC[Encryption]
  UM[User management] -->|reveal entry| ENC[Encryption]
  ENC -->|ciphertext| ST[Storage] -->|header + ciphertext| ENC
  ENC -->|entries| CLI; ST -->|ciphertext copy| BK[Backup]
```
...
**Vault format.** Vault is a single file: plaintext header + one encrypted body, header authenticated as associated data.

Header contains format version, KDF id and params, salt, wrapped vault key, revision counter, body nonce. Body is a compressed JSON of all entries, padded to 4 KiB, rewritten whole on every save. Per-entry encryption leaks entry count and sizes and multiplies nonce management, wchih gains nothing in performance, since the app is so small.
...
3) Master password and vault key can be rotated independently.
...
- **An attacker has access to encrypted vault file.**
- **An attacker has access to the application source code.**
- **An attacker can perform read memory on target device after the vault has been unlocked and used and is locked again.**
...
Threats arising from the loopback HTTP surface and the browser client are out of scope here and covered in `web-app-security.md` §4.
...
| Vault at rest | Prior ciphertext left on the storage medium after rename | Atomic rename does not erase old blocks on SSDs or copy-on-write filesystems. So, the vault key needs to be rotated for every change. |
...
| Vault in memory | Key paged to swap or a hibernation file | `mlock` the key material and the decrypted entry set; no swap allocations for either |
